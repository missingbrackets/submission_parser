...
#  Skill Editor page — Streamlit UI.
...
""", unsafe_allow_html=True)
    ...
    with tab_fields:
        ...
        for f in visible_fields:
            ...
            with st.expander(f"{f.key}  ·  {f.label}  ·  {f.section.value}", expanded=False):
                st.markdown(f'**{f.key}** {_badge(is_new, is_ov)}', unsafe_allow_html=True)
                ...
                with col_a:
                    new_label = st.text_input("Label", value=f.label, key=f"editor_label_{f.key}")
                with col_b:
                    new_section = st.selectbox(
                        "Section / Tab", options=SECTION_OPTIONS,
                        index=SECTION_OPTIONS.index(f.section.name),
                        format_func=_section_display, key=f"editor_section_{f.key}",
                    )
                with col_c:
                    new_type = st.selectbox(
                        "Field Type", options=FIELD_TYPE_OPTIONS,
                        index=FIELD_TYPE_OPTIONS.index(f.field_type.name),
                        key=f"editor_type_{f.key}",
                    )

                with col_d:
                    new_in_csv = st.checkbox("In Submission CSV", value=f.in_csv, key=f"editor_csv_{f.key}")
                with col_e:
                    new_in_summary = st.checkbox("Show in Tab", value=f.in_summary, key=f"editor_sum_{f.key}")
                with col_f:
                    new_gap = st.checkbox("Gap Check", value=f.gap_check, key=f"editor_gap_{f.key}")

                new_critical = st.checkbox(
                    "🔴 Critical (missing = RED flag)", value=f.critical, key=f"editor_crit_{f.key}",
                )

                with col_g:
                    new_desc = st.text_input("Description / tooltip", value=f.description, key=f"editor_desc_{f.key}")

                apply_col, remove_col = st.columns([3, 1])
                with apply_col:
                    if st.button(f"✅ Apply changes to '{f.key}'", key=f"editor_apply_{f.key}", use_container_width=True):
                        ov = {
                ...
                with remove_col:
                    if is_new:
                        if st.button(f"🗑 Remove", key=f"editor_remove_{f.key}", use_container_width=True):
                            working["field_additions"] = [
                                a for a in working.get("field_additions", []) if a.get("key") != f.key
                            ]
                            st.session_state[_sk] = working
                            st.rerun()
                    elif is_ov:
                        if st.button(f"↩ Revert", key=f"editor_revert_{f.key}", use_container_width=True):
        ...
        with na1:
            new_key_mode = st.radio(
                ...
                key="editor_new_key_mode", horizontal=True,
            )
        with na2:
        ...
        with nb1:
            nf_label = st.text_input("Label", value=_prefill.label if _prefill else "", key="editor_nf_label")
        with nb2:
            nf_section = st.selectbox(
                "Section / Tab", options=SECTION_OPTIONS,
                index=SECTION_OPTIONS.index(_prefill.section.name) if _prefill else 0,
                format_func=_section_display, key="editor_nf_section",
            )
        with nb3:
    ...
    with tab_csv:
        ...
        for csv_key, csv_display in CSV_FILES.items():
            ...
            new_consol[csv_key] = st.checkbox(
                ...
                value=default_val, key=f"editor_consol_{csv_key}",
            )
    ...
    with tab_prompt:
        ...
        edited_prompt = st.text_area(
            "System prompt", value=current_prompt, height=500,
            key="editor_prompt_text", label_visibility="collapsed",
        )
        ...
        with pa2:
            if has_prompt_override:
                if st.button("↩ Revert to .py Default", key="editor_revert_prompt", use_container_width=True):
    ...
    with tab_preview:
        ...
        if st.button("💾 Save All Changes", type="primary", key="preview_save_btn", use_container_width=False):
            saved_path = save_config(skill_code, working)
            st.success(f"✅ Saved to `{saved_path}`")
            st.session_state[_sk] = copy.deepcopy(working)
            st.rerun()
